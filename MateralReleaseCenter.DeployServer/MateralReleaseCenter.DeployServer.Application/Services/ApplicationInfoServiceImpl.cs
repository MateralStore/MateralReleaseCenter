...
public partial class ApplicationInfoServiceImpl(IServiceProvider serviceProvider, IOptionsMonitor<ApplicationConfig> config)
{
    ...
    {
        if (MateralServices.Services == null) throw new MateralReleaseCenterException("获取DI容器失败");
    ...
    {
        if (await DefaultRepository.ExistedAsync(m => m.Name == model.Name)) throw new MateralReleaseCenterException("名称重复");
        if (await DefaultRepository.ExistedAsync(m => m.RootPath == model.RootPath)) throw new MateralReleaseCenterException("根路径重复");
    ...
    {
        if (ApplicationRuntimeHost.ApplicationRuntimes[model.ID].ApplicationStatus != ApplicationStatusEnum.Stop) throw new MateralReleaseCenterException("应用程序尚未停止");
    ...
    {
        if (ApplicationRuntimeHost.ApplicationRuntimes[id].ApplicationStatus != ApplicationStatusEnum.Stop) throw new MateralReleaseCenterException("应用程序尚未停止");
    ...
    {
        if (!ApplicationRuntimeHost.ApplicationRuntimes.TryGetValue(id, out ApplicationRuntimeModel? value)) throw new MateralReleaseCenterException("应用程序不存在");
        ApplicationRuntimeModel application = value;
        ApplicationInfoDTO result = Mapper.Map<ApplicationInfoDTO>(application) ?? throw new MateralReleaseCenterException("映射失败");
    ...
    public async Task SaveFileAsync([Required(ErrorMessage = "唯一标识为空")] Guid id, [Required(ErrorMessage = "文件为空")] IFormFile file)
    {
        ...
        if (string.IsNullOrWhiteSpace(fileName)) throw new MateralReleaseCenterException("未识别文件名");
        string extension = Path.GetExtension(fileName).ToLowerInvariant();
        string[] allowedExtensions = [".rar", ".zip", ".tar", ".gz", ".bz2", ".7z"];
        if (!allowedExtensions.Contains(extension)) throw new MateralReleaseCenterException("只能上传压缩文件(.rar, .zip, .tar, .gz, .bz2, .7z)");
    ...
    public void DeleteFile([Required(ErrorMessage = "唯一标识为空")] Guid id, string fileName)
    {
        if (!ApplicationRuntimeHost.ApplicationRuntimes.TryGetValue(id, out ApplicationRuntimeModel? value)) throw new MateralReleaseCenterException("应用程序信息不存在");
        ApplicationRuntimeModel applicationInfo = value;
        FileInfo[]? fileInfos = applicationInfo.GetRarFileNames() ?? throw new MateralReleaseCenterException("文件不存在");
        FileInfo? fileInfo = fileInfos.FirstOrDefault(m => m.Name == fileName);
        if (fileInfo == null || !fileInfo.Exists) throw new MateralReleaseCenterException("文件不存在");
        try
        {
            fileInfo.Delete();
        }
        ...
        {
            throw new MateralReleaseCenterException($"文件删除失败:{ex.Message}", ex);
        }
    }
    /// <summary>
    /// 启动
